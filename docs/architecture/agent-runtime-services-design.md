...
- 权限规划按纯决策与产品编排分层：
  ...
  - `resources` 表示本次操作，`save_resources` 表示用户选择“始终允许”后保存的范围；Web 与 TUI 必须在确认前展示后者。本地工作区编辑可以保存工作区子树，外部文件保持精确文件范围；读取敏感文件的规则独立生效。
  - 本地命令仅在已确认 cwd 位于当前工作区时按完整裸命令复用授权；外部或远端 cwd 保留带目录的精确资源，无法确认 cwd 的旧 Bash 请求不保存命令授权。命令参数变化仍需重新评估。
  - “始终允许”成功保存后，同会话、同模型轮中已排队且保存范围完全被该授权覆盖的文件/命令请求可一并完成；其他会话、其他轮、其他项目、不同动作及要求重新确认的请求不随之完成。授权和这些请求的审计记录必须先原子提交，再释放等待者。
  - 子会话在模型轮边界读取直接父任务的 Turn / Session 权限选择；普通父会话清除 Session 覆盖后跟随全局默认。多级委派中，直接父任务也是子会话且没有显式选择时保留委派快照，不能用全局默认替换其继承权限。外部命令委派入口同样登记并清理单次 Turn 选择。已进入等待的请求仍遵循该轮已解析的权限，切换模式不会追认当前待确认操作。
