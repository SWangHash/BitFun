...
use crate::util::types::{GeminiResponse, GeminiUsage, ToolCall};
use anyhow::Result;
            ...
            Ok(chunk) => {
                ...
                if let Some(chunk_provider_metadata) = chunk_provider_metadata {
                    match provider_metadata.as_mut() {
                        Some(existing) => {
                            crate::infrastructure::ai::client::utils::merge_json_value(
                                existing,
...
pub(crate) fn unified_usage_to_gemini_usage(
    usage: crate::infrastructure::ai::ai_stream_handlers::UnifiedTokenUsage,
) -> GeminiUsage {
    GeminiUsage {
